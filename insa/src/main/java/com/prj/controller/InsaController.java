...
import com.prj.util.CommonService;
...
//    private UserService userSer;
    ...
    public String personForm(PersonVO personVO) {
        return "/insa/join";
    }
    
    @RequestMapping(value="/join", method = RequestMethod.POST) 
    public String join(@Valid PersonVO personVo, BindingResult bindingResult){
    ...
    public void postmailer(@RequestParam("subject") String subject, @RequestParam("mailaddr") String mailaddr, 
    		@RequestParam("fileadd") MultipartFile file, HttpSession session  ) throws IOException{
    	...
    	if(!file.getOriginalFilename().isEmpty()) {
			...
			File f = new File("C:\\temp", saveName);
			FileCopyUtils.copy(file.getBytes(),f);
			
			System.out.println("파일절대경로" + f.getCanonicalPath());
    	...
    	System.out.println("메일제목"+ subject );
    	System.out.println("메일주소"+ mailaddr );
    	CommonService mailsend = new CommonService();
    	mailsend.sendEmail(mailaddr, subject, filePath, session);
