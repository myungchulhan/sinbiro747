...
$(function(){
	$("#delChk").click(function(){
		if ($("input[id=delChk]").prop("checked")){
...
////선택반전
function gogo(){
	$('[name=del]').prop('checked', function(i, val){ return !val });
}


function goDel(){
	//alert($("input[type=form]"));
//	$("input[type=form]").action = "/gogo.do";
...
$("#form1").attr("action","/insa/gogo.do");       
$("#form1").submit();        
			...
			<table class="table table-hover">
				<thead>
					<tr>
						...
						<th>부서</th>
						<th>취미</th>
						<th><input type="button" id="submitForm" value="GO" onclick="goDel();">
						<th><input type="checkbox" id="delChk">전체
						<button type="button" onclick="gogo()">반전</button>
						<input type="button" id="selChk" value="선택삭제"></th>
					</tr>
				</thead>
				<!-- 목록 시작 -->
